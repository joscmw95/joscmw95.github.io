...
file_path = '/Users/joshuachanm/Downloads/Theme of Lutie _ Snow In My He.mp3'
...
amplitude_threshold = np.max(np.abs(filtered_samples)) * 0.01  # Lower the threshold for more beats
...
for i, time in enumerate(filtered_peak_times):
    ...
    if 30 < time < 60:  # Assume this is a build-up section
        # Favor lane 1 for buildup
        if lane is None or random.random() < 0.7:  # 70% chance for lane 1 during buildup
            lane = 1
    
    # Add the note to the beatmap (only if a lane is selected)
    if lane:
        beatmap.append({"time": float(round(time, 3)), "lane": lane})
